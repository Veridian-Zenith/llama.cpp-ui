...
import {
  X, ChevronDown, Thermometer, Gauge, Cpu, Shield, Zap, Sliders, Terminal,
  Globe, Search, Brain, Lock, Unlock, AlertTriangle,
  BarChart3, Cloud, Hash, User, Sparkles,
  Wifi, WifiOff,
} from 'lucide-react';
  ...
  return (
    <div className="border-b border-[var(--vz-border-color)]/30 last:border-b-0">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2.5 text-[var(--vz-text-secondary)] hover:text-[var(--vz-accent-vibrant)] transition-colors cursor-pointer"
      >
        <div className="flex items-center gap-2">
          <Icon size={12} className="text-[var(--vz-accent-vibrant)]/60" />
          <span className="text-[10px] font-bold uppercase tracking-wider">{title}</span>
          {badge && (
            <span className="text-[8px] font-mono px-1.5 py-0.5 rounded-full bg-[var(--vz-accent-vibrant)]/10 text-[var(--vz-accent-vibrant)]/70">
              {badge}
            </span>
          )}
        </div>
        <motion.div animate={{ rotate: open ? 0 : -90 }} transition={{ duration: 0.15 }}>
          <ChevronDown size={12} className="text-[var(--vz-text-secondary)]/30" />
        </motion.div>
      </button>
      <AnimatePresence>
        {open && (
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] font-mono text-[var(--vz-text-secondary)]/60 uppercase tracking-wider">{label}</span>
        </div>
        <span className="text-[9px] font-mono text-[var(--vz-accent-vibrant)]">{value}</span>
      </div>
      <input
...
export function RightPanel({ rightTab, onClose }: Props) {
  const {
  ...
  return (
    <motion.div
      ...
      className="w-[340px] h-full bg-[var(--vz-bg-primary)] border-l border-[var(--vz-border-color)]/30 flex flex-col overflow-hidden shrink-0 z-20"
    >
      {rightTab === 'stats' && (
        <>
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--vz-border-color)]/20 shrink-0">
            <div className="flex items-center gap-2">
              <BarChart3 size={13} className="text-[var(--vz-accent-vibrant)]" />
              <span className="text-[11px] font-bold uppercase tracking-wider text-[var(--vz-accent-vibrant)]/80">Stats</span>
            </div>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
              onClick={onClose} className="p-1 rounded-md hover:bg-white/5 text-[var(--vz-text-secondary)]/30 cursor-pointer">
              <X size={13} />
            </motion.button>
          </div>
        ...
        <>
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--vz-border-color)]/20 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold uppercase tracking-wider text-[var(--vz-accent-vibrant)]/80">Settings</span>
              <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[8px] font-mono ${
                isConnected ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
              }`}>
                {isConnected ? <Wifi size={7} /> : <WifiOff size={7} />}
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            ...
            <Section title="Connection" icon={Wifi} defaultOpen={true}>
              <div className="space-y-2">
                ...
                <motion.button
                  ...
                  className="w-full bg-[var(--vz-accent-vibrant)]/10 hover:bg-[var(--vz-accent-vibrant)]/20 border border-[var(--vz-accent-vibrant)]/30 text-[var(--vz-accent-vibrant)] text-xs font-bold rounded-lg py-2 transition-colors cursor-pointer"
                >
                ...
                {serverCapabilities && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {serverCapabilities.supportsMiRoC && (
                      <span className="text-[8px] font-mono px-1.5 py-0.5 rounded bg-[var(--vz-accent-vibrant)]/10 text-[var(--vz-accent-vibrant)]/60 border border-[var(--vz-accent-vibrant)]/20">MiRoC</span>
                    )}
                    {serverCapabilities.supportsNativeTools && (
                      <span className="text-[8px] font-mono px-1.5 py-0.5 rounded bg-green-500/10 text-green-400/60 border border-green-500/20">Native Tools</span>
                    )}
                    {serverCapabilities.supportsGrammar && (
                      <span className="text-[8px] font-mono px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-400/60 border border-purple-500/20">Grammar</span>
                    )}
                  </div>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={`text-[10px] font-bold ${active ? 'text-[var(--vz-accent-vibrant)]' : 'text-[var(--vz-text-secondary)]/70'}`}>
                          {p.name}
                        </div>
                        <div className="text-[8px] text-[var(--vz-text-secondary)]/30 truncate">{p.description}</div>
                      </div>
                      {active && <div className="w-1.5 h-1.5 rounded-full bg-[var(--vz-accent-vibrant)] animate-pulse" />}
                    </motion.button>
                  );
                })}
              </div>
            ...
            <Section title="Tools" icon={Terminal} defaultOpen={true}>
              <div className="space-y-1.5">
                {[
                  ...
                  { name: 'Web Fetch', icon: Globe },
                  { name: 'Terminal', icon: Terminal },
                  { name: 'File I/O', icon: Cpu },
                  { name: 'Memory', icon: Brain },
                  { name: 'Profile', icon: User },
                ].map((tool) => (
                  <div key={tool.name} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-[var(--vz-bg-secondary)]/40 border border-[var(--vz-border-color)]/30">
                    <tool.icon size={9} className="text-[var(--vz-accent-vibrant)]/40" />
                    <span className="text-[9px] font-mono text-[var(--vz-text-secondary)]/50">{tool.name}</span>
                ...
                <motion.button
                  ...
                  className={`w-full flex items-center justify-center gap-2 py-1.5 rounded-lg border text-[9px] font-bold transition-colors cursor-pointer ${
                    terminalOpen
            ...
            <Section title="Sandbox" icon={sandbox.enabled ? Lock : Unlock}>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[9px] font-mono text-[var(--vz-text-secondary)]/60 uppercase tracking-wider">Sandbox</span>
                  <motion.button
                  ...
                  >
                    <motion.div
                      animate={{ x: sandbox.enabled ? 18 : 2 }}
                ...
                {!sandbox.enabled && (
                  <div className="flex items-center gap-1.5 p-1.5 rounded-lg bg-red-500/10 border border-red-500/20">
                    ...
                    <span className="text-[9px] text-red-400/70">Sandbox off — terminal runs freely</span>
                  </div>
                )}
              </div>
            </Section>
            ...
            <Section title="System Prompt" icon={Cpu}>
              <textarea
                ...
                rows={4}
                className="w-full bg-[var(--vz-bg-secondary)] border border-[var(--vz-border-color)]/50 rounded-lg px-3 py-2 text-[10px] font-mono text-[var(--vz-text-secondary)] outline-none focus:border-[var(--vz-accent-vibrant)]/40 transition-colors resize-none"
              />
            </Section>
          ...
          <div className="p-2 border-t border-[var(--vz-border-color)]/20 text-center shrink-0">
            <p className="text-[8px] font-mono text-[var(--vz-accent-muted)]/15 uppercase tracking-widest">
              llama.cpp · bun · react
            </p>
          </div>
        </>
